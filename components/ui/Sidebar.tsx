...
export function Sidebar({
  ...
  const { drawerOpen, collapsed } = useShell()

  // Collapse is desktop-only, so every class it turns on is `lg:`-prefixed and
  // the same markup serves both. See lib/shell.ts for why there is no second
  // copy of this nav for mobile.
  const hideOnRail = collapsed ? SIDEBAR.labelHidden : ''
  const railCentre = collapsed ? 'lg:justify-center lg:px-0' : ''

  return (
    <aside
      // z-40, above the backdrop's z-30. Off-canvas below `lg` and slid in by
      // the drawer state; always on screen from `lg` up, at one of two widths.
      className={
        'fixed bottom-0 left-0 z-40 flex flex-col border-r border-gray-800 bg-gray-900 ' +
        'transition-[transform,width] duration-200 lg:translate-x-0 ' +
    ...
    >
      ...
      <Link
        href={homeHref}
        className={
          'flex h-16 shrink-0 items-center gap-2.5 border-b border-gray-800 px-5 transition hover:bg-gray-800/40 ' +
          railCentre
        }
      >
        ...
        <span className={'text-lg font-bold tracking-tight text-white ' + hideOnRail}>
                ...
                return (
                  <li key={item.href}>
                    <Link
                      ...
                      // py-2.5 rather than py-1.5: on a phone this list IS the
                      // navigation, and a 26px row between two other 26px rows
                      // is a mis-tap. 44px here costs a desk user nothing.
                      title={collapsed ? item.label : undefined}
                      className={
                        'flex items-center gap-3 rounded-md border-l-2 px-3 py-2.5 text-sm transition lg:py-2 ' +
                        railCentre + ' ' +
                        (active
                    ...
                    >
                      ...
                      <span className={hideOnRail}>{item.label}</span>
                    </Link>
                  </li>
                )
              })}
      ...
      <div className="shrink-0 border-t border-gray-800 p-3">
        {/* The company chip and the name block are text with no icon to fall
            back to, so the rail drops them outright rather than clipping them.
            Both are still in the header's user menu. */}
        <div
          className={
            'mb-3 flex items-center gap-2 rounded-md bg-gray-950/60 px-3 py-2 ' + hideOnRail
          }
        >
        ...
        <div className={'mb-2 flex items-center gap-3 px-1 ' + (collapsed ? 'lg:justify-center lg:px-0' : '')}>
          <span
          ...
          <span className={'min-w-0 ' + hideOnRail}>
        ...
        <form action={signOut}>
          <button
            ...
            className={
              'flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm text-gray-400 transition hover:bg-gray-800 hover:text-red-400 lg:py-2 ' +
              railCentre
            }
          >
